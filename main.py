...
class CardsDeck(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.count_deck() > 2:
            ...
            self.image.blit(bg, (0, 0), (self.new_x, self.new_y, self.new_x + self.height, self.new_y + self.width))
...
class Message(pygame.sprite.Sprite):
    ...
    def __init__(self, message: str):
        ...
        pygame.sprite.Sprite.__init__(self)
        # Создаем поверхность
        self.width, self.height = (vw * 14, vw * 6)
        self.image = pygame.Surface((self.width, self.height))
        self.rect = self.image.get_rect()
        self.new_x, self.new_y = (5 * vw, 4 * vw)
        # Распологаем сообщение
        self.rect.topleft = (self.new_x, self.new_y)
        # Заполняем картинкой стола чтобы не было черного экрана
        self.image.blit(bg, (0, 0), (self.new_x, self.new_y, self.new_x + self.width, self.new_y + self.height))
        # Наносим текст
...
def check_win(player: Player, bot: Bot, deck: CardsDeck):
    ...
    # Если в колоде нет карт
        ...
        if not len(player.inventory):
            if len(bot.inventory) == len([i for i in table.cards if not table.cards[i]]):
                attack_cards = [i for i in table.cards if not table.cards[i]]
                deffended_cards = bot.inventory
                test = []
                for c1 in attack_cards:
                    for c2 in deffended_cards:
                        if c2 not in test and c2.__gt__(c1, DECK.trump):
                            test.append(c2)
                if len(test) == len([i for i in table.cards if not table.cards[i]]):
                    return "Ничья"
            return "Игрок"
        # Если в инвентаре бота нет карт
        elif not len(bot.inventory):
            if len(player.inventory) == len([i for i in table.cards if not table.cards[i]]):
                attack_cards = [i for i in table.cards if not table.cards[i]]
                deffended_cards = player.inventory
                test = []
                for c1 in attack_cards:
                    for c2 in deffended_cards:
                        if c2 not in test and c2.__gt__(c1, DECK.trump):
...
def main():
    ...
    global vw, sprites, player, DECK, bita, bot, table, CARDS, SUITS, CELL, GORIZONTAL_SHIRT, VERTICAL_SHIRT, \
           VALUES, CARDS_PATH, GAME_NUMBER, steps, running, COUNT_CARDS, bg, CLOCK
                    ...
                    if place.rect.collidepoint(event.pos):    
                        ...
                        if card:
                            ...
                            if place.card not in table.cards and not bot.attack_status and not place.card:
                                ...
                                player.update()
                            # Если бот атакует
                            elif place.card in table.cards and not table.cards[place.card] and bot.otbitie:
                                # Удаляем из спрайтов отображение перемещаемой карты
                                sprites.remove(prewiew) 
                                # Флаг того что карта установлена на стол
                                put_card = False
